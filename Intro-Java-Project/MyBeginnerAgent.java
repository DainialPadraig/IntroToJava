/**
 * My implementation of an agent that plays a beginner level game. It will take a potentially winning
 * move, if it sees it, it will block a potentially winning move by the opponent, and will move randomly
 * if there is no blocking move. Not an especially brilliant Connect 4 player, but should regularly beat a
 * random agent and hopefully do well against the supplied beginner agent.
 * 
 * @author Dan Stormont 
 * @version 1.0
 */
...
public class MyBeginnerAgent extends Agent
{
    private Random r;
     ...
     */
    public MyBeginnerAgent(Connect4Game game, boolean iAmRed)
    {
        super(game, iAmRed);
        this.r = new Random();
    }
    ...
    public void move()
    {
        int col = iCanWin();  // test to see if the agent has a winning move
        if (col < 0) {          // the agent doesn't have a winning move
            if ((col = theyCanWin()) < 0) { // the opponent doesn't have a winning move, either
     ...
     * 
     * This is very naive because it will only recognize the end move when it sees it, rather than
     * planning moves to get there.
     *
